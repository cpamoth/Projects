    ...
    public class Inspector
    {
        ...
        private void InspectDocument(XmlDocument doc)
        {
            ...
            if (XmlNodeType.XmlDeclaration == doc.FirstChild.NodeType)
            {
                ...
                if (0 != String.Compare("utf-8", declaration.Encoding, true))
                {
                    ...
                    else
                    {
                        switch (node.LocalName)
                        {
                            ...
                            case "FileSearch":
                                ...
                                break;
                            case "FragmentRef":
                                this.InspectFragmentRefElement((XmlElement)node);
                                break;
                            case "Icon":
                                this.InspectIconElement((XmlElement)node);
                                break;
                            case "IgnoreModularization":
                                node = this.InspectIgnoreModularizationElement((XmlElement)node);
                                break;
                            case "Include":
                                node = this.InspectIncludeElement((XmlElement)node);
                                break;
                            case "IniFile":
                                this.InspectIniFileElement((XmlElement)node);
                                break;
                            case "IniFileSearch":
                                this.InspectIniFileSearchElement((XmlElement)node);
                                break;
                            case "Media":
                            ...
                            case "Merge":
                                ...
                                break;
                            case "Module":
                                this.InspectModuleElement((XmlElement)node);
                                break;
                            case "Package":
                                this.InspectPackageElement((XmlElement)node);
                                break;
                            case "PatchSequence":
                                this.InspectPatchSequenceElement((XmlElement)node);
                                break;
                            case "Permission":
                                node = this.InspectPermissionElement((XmlElement)node);
                                break;
                            case "Product":
                                this.InspectProductElement((XmlElement)node);
                                break;
                            case "ProgId":
                                this.InspectProgIdElement((XmlElement)node);
                                break;
                            case "ProgressText":
                            ...
                            case "RadioGroup":
                                ...
                                break;
                            case "Registry":
                                node = this.InspectRegistryElement((XmlElement)node);
                                break;
                            case "RegistrySearch":
                                this.InspectRegistrySearchElement((XmlElement)node);
                                break;
                            case "RemoveFile":
                                this.InspectRemoveFileElement((XmlElement)node);
                                break;
                            case "ServiceInstall":
                            ...
                            case "TypeLib":
                                ...
                                break;
                            case "Upgrade":
                                this.InspectUpgradeElement((XmlElement)node);
                                break;
                            case "UpgradeImage":
                            ...
                            case "UpgradeVersion":
                                ...
                                break;
                            case "Verb":
                                this.InspectVerbElement((XmlElement)node);
                                break;
                            case "WebApplicationExtension":
                                node = this.InspectWebApplicationExtensionElement((XmlElement)node);
                                break;
                            case "WebFilter":
                                node = this.InspectWebFilterElement((XmlElement)node);
                                break;
                            case "Wix":
                                node = this.InspectWixElement((XmlElement)node);
                                break;
                            case "WixLocalization":
                                node = this.InspectWixLocalizationElement((XmlElement)node);
                                break;
        ...
        private void InspectComponentElement(XmlElement element)
        {
                ...
                null != driverLegacy || null != driverPlugAndPlayPrompt || null != driverSequence)
            {
                ...
                if (this.OnError(InspectorTestType.NamespaceChanged, element, String.Format("The Component/@Driver* attributes are now set via the Driver element which is part of the DifxApp extension.  An xmlns:difxapp=\"http://schemas.microsoft.com/wix/DifxAppExtension\" attribute should be added to the Wix element and these attributes should be moved to a 'difxapp:Driver' element without the 'Driver' prefix.")))
                {
        ...
        private void InspectProgIdElement(XmlElement element)
        {
            ...
            if (null != icon)
            {
                ...
                if (icon.Value.StartsWith("\"[") && icon.Value.EndsWith("]\""))
                {
                    icon.Value = icon.Value.Substring(1, icon.Value.Length - 2);
                }

                if ((icon.Value.StartsWith("[!") || icon.Value.StartsWith("[#")) && icon.Value.EndsWith("]"))
                {
        ...
        private void InspectVerbElement(XmlElement element)
        {
            ...
            if (null != target)
            {
                ...
                {
                    // remove quotes first if they are present
                    if (target.Value.StartsWith("\"[") && target.Value.EndsWith("]\""))
                    {
        ...
        private XmlElement InspectHelpElement(XmlElement element)
        {
            ...
            if (this.OnError(InspectorTestType.NamespaceChanged, element, String.Format("The {0} element is now part of the VS extension.  An xmlns:vs=\"http://schemas.microsoft.com/wix/VSExtension\" attribute should be added to the Wix element and this element should be renamed to 'vs:{0}'.", element.LocalName)))
            {
        ...
        private XmlElement InspectIIsElement(XmlElement element)
        {
            if ("http://schemas.microsoft.com/wix/IIsExtension" != element.NamespaceURI)
            {
                ...
                if (this.OnError(InspectorTestType.NamespaceChanged, element, String.Format("The {0} element is now part of the IIS extension.  An xmlns:iis=\"http://schemas.microsoft.com/wix/IIsExtension\" attribute should be added to the Wix element and this element should be renamed to 'iis:{0}'.", element.LocalName)))
                {

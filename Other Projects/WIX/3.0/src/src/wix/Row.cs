...
namespace Microsoft.Tools.WindowsInstallerXml
{
    ...
    public class Row
    {
        ...
        internal string GetModularizedValue(Field field, string modularizationGuid, Hashtable suppressModularizationIdentifiers)
        {
            ...
            if (null != modularizationGuid && ColumnModularizeType.None != field.Column.ModularizeType && !(Installer.IsStandardAction(fieldData) || Installer.IsStandardProperty(fieldData)))
            {
                StringBuilder sb;
                int start;
                    ...
                    case ColumnModularizeType.Condition:
                        ...
                        for (int i = matches.Count - 1; 0 <= i; i--)
                        {
                            ...
                            if (group.Success)
                            {
                                ...
                                if (!Installer.IsStandardProperty(identifier) && (null == suppressModularizationIdentifiers || !suppressModularizationIdentifiers.Contains(identifier)))
                                {

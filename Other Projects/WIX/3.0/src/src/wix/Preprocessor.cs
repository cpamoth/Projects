...
namespace Microsoft.Tools.WindowsInstallerXml
{
    ...
    {
        private static Regex defineRegex = new Regex(@"^\s*(?<varName>.+?)\s*=\s*(?<varValue>.+?)\s*$", RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.ExplicitCapture);
        ...
        public XmlDocument Process(string sourceFile, Hashtable variables)
        {
            ...
            using (StringReader sr = new StringReader(processed.ToString()))
            {
                ...
                {
                    this.core.OnMessage(WixErrors.MissingXmlnsAttributeOrSP1());
                }
                finally
                {
                    if (null != reader)
        ...
        private string PreprocessVariables(string value)
        {
            ...
            while (-1 != (i = value.IndexOf('$', end)))
            {
                ...
                else if (remainder.StartsWith("(env."))
                {
                    ...
                    if (-1 == i)
                    {
                        throw new WixException(WixErrors.UnmatchedParenthesisInExpression(this.GetCurrentSourceLineNumbers(), String.Format("Ill-formed environment variable in expression: '${0}'", remainder)));
                    }
                    ...
                    if (null == envVariable)
                    {
                        throw new WixException(WixErrors.InvalidPreprocessorVariable(this.GetCurrentSourceLineNumbers(), String.Format("Undefined environment variable: $(env.{0}).", variable)));
                    }
                ...
                else if (remainder.StartsWith("(var.") || remainder.StartsWith("(sys."))
                {
                    ...
                    if (-1 == i)
                    {
                        throw new WixException(WixErrors.UnmatchedParenthesisInExpression(this.GetCurrentSourceLineNumbers(), String.Format("Ill-formed variable in expression: '${0}'", remainder)));
                    }

                    string variable = remainder.Substring(1, i - 1).Trim();
                    string varParameter = this.core.GetVariableValue(this.GetCurrentSourceLineNumbers(), variable, false);
                    if (null == varParameter)
                    {
                        throw new WixException(WixErrors.InvalidPreprocessorVariable(this.GetCurrentSourceLineNumbers(), String.Format("Undefined variable: $({0}).", variable)));
                    }
                ...
                else if (remainder.StartsWith("(loc."))
                {
                    ...
                    if (-1 == i)
                    {
                        throw new WixException(WixErrors.UnmatchedParenthesisInExpression(this.GetCurrentSourceLineNumbers(), String.Format("Ill-formed resource variable in expression: '${0}'", remainder)));
                    }
                ...
                else if (remainder.StartsWith("("))
                {
                    ...
                    if (null == result)
                    {
                        throw new WixException(WixErrors.InvalidPreprocessorVariable(this.GetCurrentSourceLineNumbers(), String.Format("Undefined parameter: $({0}).", parameterName)));
                    }
                    sb.Append(result);
                    end += i + 1;
                }
        ...
        private string TryExtensionPreprocessParameter(string parameterName)
        {
            ...
            if (0 > indexOfPeriod)
            {
                this.core.OnMessage(WixErrors.PreprocessorMissingParameterPrefix(this.GetCurrentSourceLineNumbers(), parameterName));
            }
            ...
            if (null == extension)
            {
                this.core.OnMessage(WixErrors.PreprocessorExtensionForParameterMissing(this.GetCurrentSourceLineNumbers(), parameterName, prefix));
            }
        ...
        {
            return (PreprocessorExtension)this.extensionsByPrefix[extensionType];
        }
        ...
        private void PreprocessDefine(string originalDefine)
        {
            ...
            if (1 < defineValue.Length && ((defineValue.StartsWith("\"") && defineValue.EndsWith("\"")) || (defineValue.StartsWith("'") && defineValue.EndsWith("'"))))
            {
        ...
        private void PreprocessInclude(string includePath, XmlWriter writer)
        {
            ...
            if (null == includeFile)
            {
                throw new WixException(WixErrors.FileNotFound(this.GetCurrentSourceLineNumbers(), includePath, "include"));
            }
            ...
            using (Stream includeStream = new FileStream(includeFile.FullName, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                ...
                {
                    this.UpdateInformation(reader, 0);
                    throw new WixException(WixErrors.SP1ProbablyNotInstalled());
                }
        ...
        private void PreprocessForeach(XmlReader reader, XmlWriter writer, int offset)
        {
            ...
            {
                throw new WixException(WixErrors.PreprocessorIllegalForeachVariable(this.GetCurrentSourceLineNumbers(), varName));
            }
            ...
            // restrict the foreach to contain only one Fragment element
            if ("Fragment" != reader.LocalName)
            {
                throw new WixException(WixErrors.IllegalForeachElement(this.GetCurrentSourceLineNumbers(), reader.Name));
            }
            ...
            {
                this.core.AddVariable(this.GetCurrentSourceLineNumbers(), varName, varValues[i]);
        ...
        {
            foreach (PreprocessorExtension preprocessorExtension in this.extensions)
            {
        ...
        private bool EvaluateAtomicExpression(string originalExpression, ref string expression)
        {
            ...
            else
            {
                ...
                {
                    if (0 == String.Compare(leftValue, rightValue, true, CultureInfo.InvariantCulture))
                    {
                        expressionValue = true;
                    }
                }
        ...
        private FileInfo GetIncludeFile(string includePath)
        {
            ...
            includePath = this.core.PreprocessString(this.GetCurrentSourceLineNumbers(), includePath);
            ...
            else // relative path
            {
                ...
                string includeTestPath = String.Concat(this.core.GetVariableValue(this.GetCurrentSourceLineNumbers(), "sys", "SOURCEFILEDIR"), includePath);

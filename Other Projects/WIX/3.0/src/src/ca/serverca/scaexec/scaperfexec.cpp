...
//    Assert(FALSE);
    ...
    if (NULL == pfnPerfCounterTextString)
        ExitOnLastError(hr, "failed to get DLL function for PerfMon");
    ...
    )
{
    ...
    while (S_OK == (hr = WcaReadStringFromCaData(&pwz, &pwzName)))
    {
        ...
        if (fInstall)
        {
            ...
            hr = CreateDataFile(pwzTempFolder, pwzName, pwzIniData, TRUE, &hIniData, &pwzIniFile);
            ExitOnFailure1(hr, "Failed to create .ini file for performance counter category: %S", pwzName);

            hr = CreateDataFile(pwzTempFolder, pwzName, pwzConstantData, FALSE, &hConstantData, NULL);
    ...
    )
{
    ...
    hr = StrAllocConcat(&pwzFile, wzName, 0);

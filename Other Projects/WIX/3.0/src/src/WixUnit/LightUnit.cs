        ...
        public static void RunUnitTest(XmlElement element, UnitResults previousUnitResults, bool update)
        {
            ...
            foreach (XmlNode node in element.ChildNodes)
            {
                if (node.NodeType == XmlNodeType.Element)
                {
                    ...
                    {
                        case "LibraryFile":
                            string libraryFile = Environment.ExpandEnvironmentVariables(node.InnerText.Trim());

                            commandLine.AppendFormat(" \"{0}\"", libraryFile);
                            break;
                        case "LocalizationFile":
                            string localizationFile = Environment.ExpandEnvironmentVariables(node.InnerText.Trim());
            ...
            {
                outputFile = "ShouldNotBeCreated.msi";
            }

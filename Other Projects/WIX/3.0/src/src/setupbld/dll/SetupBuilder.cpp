...
static HRESULT GetProductCodeAndVersionAndAllUsers(
    ...
    __in LPWSTR wzProductCode,
    __in DWORD cchProductCode,
    __in LPWSTR wzProductVersion,
    ...
    )
{
    ...
    //
    cbSetupChain = sizeof(SETUP_INSTALL_CHAIN) + (sizeof(SETUP_INSTALL_PACKAGE) * (cPackages - 1)) + (sizeof(SETUP_INSTALL_TRANSFORM) * (cTransforms - 1));
    ...
    SETUP_INSTALL_TRANSFORM* pTransform = pSetupChain->rgTransforms;
...
//        hr = StrAnsiAllocString(&pszProductName, wzProductName, 256, CP_ACP);
//        ExitOnFailure1(hr, "Failed to convert to ansi product name: %S", wzProductName);

//        if (!::UpdateResourceA(hUpdate, RT_STRING, MAKEINTRESOURCE(SETUP_RESOURCE_IDS_PRODUCTNAME), MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), pszProductName, lstrlen(pszProductName) + 1))
        DWORD cbProductName = (lstrlenW(wzProductName) + 1) * sizeof(WCHAR);
        A a = { lstrlenW(wzProductName) + 1, wzProductName };
        DWORD cba = sizeof(a.f) + cbProductName;
        if (!::UpdateResourceA(hUpdate, RT_STRING, MAKEINTRESOURCE(1), MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), (LPVOID)&a, cba))
        {
            ExitWithLastError1(hr, "Failed to update resource with product name: %S", wzProductName);
        }
    }
    ...
    )
{
    ...
    CREATE_SETUP_TRANSFORMS createSetupTransform[1];
    createSetupPackage[0].wzSourcePath = wzMsi;
    createSetupPackage[0].fPrivileged = FALSE;
    createSetupPackage[0].fCache = TRUE;

    hr = CreateSetup(wzSetupStub, NULL, NULL, NULL, createSetupPackage, createSetupTransform, countof(createSetupPackage), NULL, wzOutput);
